...
public class OrbitUniversal: MonoBehaviour, INbodyInit, IFixedOrbit, IOrbitPositions, IOrbitScalable
{
    ...
    /// <summary>
    /// For ellipses can specify size using major-axis. Typically used by editor script, bit may have other uses.
    /// </summary>
    /// <param name="a"></param>
    ...
    private void RVtoCOEWrapper() {
        ...
        OrbitUtils.OrbitElements oe = OrbitUtils.RVtoCOE(r0, v0, centerNbody, true);
        p = oe.p;
	...
	*    vallado       2013, 93, alg 8, ex 2-4
    ...
    public Vector3[] OrbitPositions(int numPoints, Vector3 centerPos, bool doSceneMapping, float hyperRadius) {
        ...
        if (double.IsNaN(eccentricity)) {
            ...
            positions = EllipsePositions(numPoints, centerPos, true); 
        } else {
            positions = HyperSegmentSymmetric(numPoints, centerPos, hyperRadius, true);
        }
        return positions;
    }
    ...
    *********************************************************************************************/
    ...
    private Vector3 GetPointForTheta(float theta, Vector3 centerPos, bool doSceneMapping) {
        Vector3 point = PositionForTheta(theta, centerPos);
    ...
    private Vector3 PositionForTheta(float theta, Vector3 centerPos) {
        ...
        if (Mathf.Abs(ecc - 1f) < 1E-6)
            ecc = (float)(1 + 1E-5);
        float r = (float)(p / (1f + ecc * Mathf.Cos(theta)));
    ...
    public Vector3[] GetPositionsForRadius(double radius, Vector3 centerPos) {
        ...
        return new Vector3[2] { PositionForTheta(theta, centerPos), PositionForTheta(-theta, centerPos) };
    ...
    private void DrawHyperGizmo(Vector3 centerPos) {
        ...
        // Draw the axes in a different color
        Gizmos.color = Color.red;
        Gizmos.DrawLine(PositionForTheta(0.5f * Mathf.PI, centerPos), 
                        PositionForTheta(-0.5f * Mathf.PI, centerPos));
        Gizmos.color = Color.blue;
