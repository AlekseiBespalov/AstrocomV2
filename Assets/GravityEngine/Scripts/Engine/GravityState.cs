...
public class GravityState
{
    ...
    private int fixedBodiesInIntegrator = 0;
...
#pragma warning restore 162
    ...
    public void AddNBody(NBody nbody, Vector3 physicsPosition, Vector3 vel_phys, float massScale) {
        ...
        if ((info[numBodies] & GravityEngine.FIXED_MOTION) > 0) {
            fixedBodiesInIntegrator++;
        } else {
            onRails = false;
        }
        numBodies++;
    ...
    public void RemoveNBody(NBody nbody) {
        if ((info[nbody.engineRef.index] & GravityEngine.FIXED_MOTION) > 0) {
            fixedBodiesInIntegrator--;
        }
    ...
    public void UpdateOnRails() {
        if (!onRails) {
            if ((fixedBodiesInIntegrator == numBodies) &&
    ...
    ********************************************/
    ...
    public void SetVelocityDouble(NBody nbody, ref double[] velocity) {
        if (nbody.engineRef.bodyType == GravityEngine.BodyType.MASSLESS) {
            ...
            // only OrbitUniversal can set a velocity
            ...
            if (orbitU != null) {
                ...
                Debug.LogWarning("Cannot change velocity of fixed body unless it is OrbitUniversal");
            }
